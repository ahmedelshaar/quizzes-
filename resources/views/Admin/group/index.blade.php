...
@section('content')
    <div class="container-fluid page__heading-container">
        <div
            ...
            <a href="{{ route('group.create') }}" class="btn btn-success ml-lg-3">اضافة مجموعة جديد <i
        ...
        <div class="table-responsive border-bottom">
            ...
            <table id="table" class="table table-striped" style="width:100%">
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ route('group.show', $group->id) }}"><button type="button" class="btn btn-primary">عرض</button></a>
                        <a href="{{ route('group.edit', $group->id) }}"><button type="button" class="btn btn-secondary">تعديل</button></a>
                        <button action="{{ route('group.destroy', $group->id) }}" name="{{$group->name}}" data-toggle="modal" data-target="#modal-standard" type="button" class="btn btn-danger delete-item-table">حذف</button>
                    </td>
                </tr>
                @endforeach
                </tbody>
...
<script>
    $(document).ready(function () {
        ...
        $('#modal-standard').on('show.bs.modal', function (event) {
            $(this).find('p').text(`انت تقوم بحذف مجموعة: ${event.relatedTarget.name}`)
