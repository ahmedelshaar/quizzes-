...
@section('content')
    <div class="container-fluid page__heading-container">
        <div
            ...
            <a href="{{ route('schedule.create') }}" class="btn btn-success ml-lg-3">اضافة تكليف جديد <i
        ...
        <div class="table-responsive border-bottom">
            ...
            <table id="table" class="table table-striped" style="width:100%">
                ...
                @foreach($schedules as $schedule)
                    <tr>
                        <td>{{$schedule['id']}}</td>
                        <td>{{$schedule['user_group']['name']}}</td>
                        <td>{{$schedule['quiz']['title']}}</td>
                        <td>{{$schedule['start']}}</td>
                        <td>{{$schedule['end']}}</td>
                        <td>
                            <a href="{{ route('schedule.show', $schedule['id']) }}"><button type="button" class="btn btn-primary">عرض</button></a>
                            <a href="{{ route('schedule.edit', $schedule['id']) }}"><button type="button" class="btn btn-secondary">تعديل</button></a>
                            <button action="{{ route('schedule.destroy', $schedule['id']) }}" name="{{$schedule['quiz']['title'] . ' من المجموعة ' . $schedule['user_group']['name'] }}" data-toggle="modal" data-target="#modal-standard" type="button" class="btn btn-danger delete-item-table">حذف</button>
                        </td>
                    </tr>
                @endforeach
                </tbody>
