                    ...
                    <form method="post" action="{{ route('schedule.store') }}">
                        <div class="form-row">
                            <div class="col-12 col-md-6 mb-3">
                                ...
                                <select id="select01" name="user_group_id" data-toggle="select" class="form-control" required>
                                    <option value="" @if(!old('user_group_id')) selected @endif>اختيار مجموعة</option>
                                    @foreach($groups as $group)
                                        <option value="{{$group->id}}"
                                                @if(old('user_group_id') == $group->id)
                                                selected
                                            @endif
                                        >{{$group->name}}</option>
                                    @endforeach
                        ...
                        <div class="form-row">
                            <div class="col-12 col-md-6 mb-3">
                                ...
                                <input type="text" name="start" id="start" class="form-control" data-toggle="flatpickr" data-flatpickr-enable-time="true" data-flatpickr-alt-format="Y-m-d h:i K" data-flatpickr-date-format="Y-m-d H:i" required>
                            </div>
                            <div class="col-12 col-md-6 mb-3">
                                <label>وقت النهاية</label>
                                <input type="text" name="end" id="end" class="form-control" data-toggle="flatpickr" data-flatpickr-enable-time="true" data-flatpickr-alt-format="Y-m-d h:i K" data-flatpickr-date-format="Y-m-d H:i" required>
                            </div>
                            <div class="col-12 col-md-6 mb-3">
                                <label>وقت ظهور الاجابة</label>
                                <select id="answer_time" data-toggle="select" class="form-control">
                                    <option value="end_quiz">ظهور الاجابة بعد انتهاء وقت الامتحان</option>
                                    <option value="end_student">تظهر الإجابة للطالب فور انتهائه</option>
                                    <option value="time_show_answer">ظهور الإجابة بعد وقت محدد</option>
                                </select>
                            </div>
                            <div class="col-12 col-md-6 mb-3 d-none" id="div_time">
                                <label>وقت ظهور الاجابة</label>
                                <input type="text" id="show_answer_time" name="show_answer_time" class="form-control" data-toggle="flatpickr" data-flatpickr-enable-time="true" data-flatpickr-alt-format="Y-m-d h:i K" data-flatpickr-date-format="Y-m-d H:i" required value="{{ \Carbon\Carbon::now() }}">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="col-12 col-md-6 mb-3">
                                <label>وقت السماح لدخول الامتحان</label>
                                <input type="number" min="5" id="grace_period" name="grace_period" class="form-control" value="5" required>
                            </div>
                        </div>
